...
class IntegerListTest(TestCase):
    ...
    def setUp(self):
        self.integer_list = IntegerList(1, 2, 3, '50', False)

    def test_correct_initialization_and_get_data(self):
        self.assertEqual([1, 2, 3], self.integer_list._IntegerList__data)

    def test_add_operation_adds_integer_to_the_list(self):
        expected_data = self.integer_list.get_data() + [4]
        actual_data = self.integer_list.add(4)
        self.assertEqual(expected_data, actual_data)

    def test_add_operation_with_float_element_raises_value_error(self):
        with self.assertRaises(ValueError) as ve:
            self.integer_list.add(4.2)
    ...
    def test_insert_integer_with_invalid_index_raises_index_error(self):
        with self.assertRaises(IndexError) as ie:
    ...
    def test_insert_float_element_on_valid_index_raises_value_error(self):
        with self.assertRaises(ValueError) as ve:
            self.integer_list.insert(0, 0.5)
